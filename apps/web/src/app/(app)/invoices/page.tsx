...
import { invoiceStatuses, type InvoiceStatus } from '@/lib/validation/invoice';
...
export default async function InvoicesPage(props: PageProps<'/invoices'>) {
  ...
  const statusFilter =
    ...
    (invoiceStatuses as readonly string[]).includes(searchParams.status)
        ...
        <Table>
          <TableHeader>
            <TableRow>
              ...
              <TableHead className="text-right">{t('fields.balanceDue')}</TableHead>
            ...
            {rows.length === 0 ? (
              ...
              rows.map((r) => (
                <TableRow key={r.id}>
                  ...
                  <TableCell className="text-right tabular-nums">
                    {formatMoney(Number(r.balance_due), { currency: r.currency, locale })}
                  </TableCell>
