...
function getAssistantModel() {
  return env.OPENAI_ASSISTANT_MODEL?.trim() || DEFAULT_OPENAI_ASSISTANT_MODEL;
}
...
function getToolCall(payload: OpenAiResponsePayload): AssistantToolCall | null {
  ...
  return { name: item.name as AssistantToolName, arguments: parsedArguments };
}
...
export async function planAssistantTool(args: {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      if (!toolCall && !message) return { ok: false, errorCode: 'invalid_response' };
