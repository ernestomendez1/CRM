import { getAssistantCopy } from './assistant-copy';
import { planAssistantTool } from './assistant-planner';
import {
...
import { loadBusinessDefaults, type BusinessDefaults } from '../domain/business';
import { createCustomerRecord, searchCustomers } from '../domain/customers';
import { createExpenseRecord, searchExpenses } from '../domain/expenses';
import { createProductRecord, searchProducts } from '../domain/products';
...
function buildMissingFieldsMessage(
  ...
  return {
    ok: true,
    type: 'clarification',
    message: copy.missingFields(action, fields.map((f) => asFieldLabel(locale, f))),
...
function buildValidationErrorMessage(
  ...
  return {
    ok: true,
    type: 'clarification',
    message: copy.invalidFields(invalidFields.map((f) => asFieldLabel(locale, f))),
    missingFields: invalidFields,
  };
}

function prepareCreateProductAction(args: { locale: Locale; raw: unknown }): AssistantChatResponse {
...
function inferCustomerFieldsFromMessage(message: string | undefined) {
  ...
  if (!source) return { name: undefined, taxId: undefined };
  ...
  return { name: remainder || undefined, taxId: taxId || undefined };
}

function prepareCreateCustomerAction(args: {
  locale: Locale;
  raw: unknown;
  message?: string;
}): AssistantChatResponse {
...
function prepareCreateExpenseAction(args: {
  locale: Locale;
...
export async function handleAssistantChat(
      ...
      plan.errorCode === 'missing_api_key'
        ...
        : plan.errorCode === 'provider_quota'
          ...
          : plan.errorCode === 'invalid_response'
            ...
            : copy.providerError;
    return { ok: false, errorCode: plan.errorCode, message };
  }

  if (!plan.toolCall) {
    return { ok: true, type: 'message', message: plan.message ?? copy.unsupported };
  }
  ...
  switch (plan.toolCall.name) {
    ...
    default:
      return { ok: true, type: 'message', message: copy.unsupported };
  }
}
