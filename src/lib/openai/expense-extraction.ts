...
const DEFAULT_OPENAI_EXPENSE_MODEL = 'gpt-5';
...
export type ExpenseExtractionSuccess = {
  ok: true;
  extracted: ExpenseExtractionDraft;
  warnings: ExpenseExtractionWarningCode[];
};

export type ExpenseExtractionFailure = {
  ok: false;
  errorCode: ExpenseExtractionErrorCode;
};

export type ExpenseExtractionResult = ExpenseExtractionSuccess | ExpenseExtractionFailure;
...
type OpenAiFileUploadResult = {
  id: string;
};

function getOpenAiExpenseModel() {
  return process.env.OPENAI_EXPENSE_MODEL?.trim() || DEFAULT_OPENAI_EXPENSE_MODEL;
}
    ...
    return [
      {
        ...
        content: [
          ...
          {
            type: 'input_text',
            text: 'Extract expense receipt fields from this PDF.',
          },
        ],
  ...
  return [
    {
      ...
      content: [
        ...
        {
          type: 'input_text',
          text: 'Extract expense receipt fields from this image.',
        },
      ],
      cleanupFileId: null,
    },
